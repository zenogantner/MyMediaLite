...
namespace Tests.RatingPrediction
{
	...
	public class SigmoidCombinedAsymmetricFactorModelTest
	{
		...
		{
			var afm = new SigmoidCombinedAsymmetricFactorModel() { LearnRate = 1.1f, Interactions = TestUtils.CreateRatings() };
		...
		{
			var afm = new SigmoidCombinedAsymmetricFactorModel() { LearnRate = 1.1f, NumIter = 10, Interactions = TestUtils.CreateRatings() };
			afm.Train();
			Assert.AreEqual(1.1f, afm.current_learnrate);
		}
		...
		{
			var afm = new SigmoidCombinedAsymmetricFactorModel() { Interactions = TestUtils.CreateRatings() };
			afm.InitModel();
