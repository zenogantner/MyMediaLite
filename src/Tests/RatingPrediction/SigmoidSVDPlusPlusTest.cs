...
{
	[TestFixture()]
	public class SigmoidSVDPlusPlusTest
	{
		[Test()]
		public void TestCurrentLearnRate()
		{
			var mf = new SigmoidSVDPlusPlus() { LearnRate = 1.1f, Interactions = TestUtils.CreateRatings() };
		...
		{
			var mf = new SigmoidSVDPlusPlus() { LearnRate = 1.1f, NumIter = 10, Interactions = TestUtils.CreateRatings() };
			mf.Train();
			Assert.AreEqual(1.1f, mf.current_learnrate);
		}
		...
		{
			var svdpp = new SigmoidSVDPlusPlus() { Interactions = TestUtils.CreateRatings() };
			svdpp.InitModel();
