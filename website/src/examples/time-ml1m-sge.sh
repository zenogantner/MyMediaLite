...
do
  params="--training-file=ml1m-0.train.txt --test-file=ml1m-0.test.txt --data-dir=$data_dir --recommender-options=\"bold_driver=true num_factors=$k num_iter=0 learn_rate=0.05 regularization=0.1 bias_reg=0.005\" --rating-type=byte --random-seed=1 --find-iter=1 --max-iter=100"
